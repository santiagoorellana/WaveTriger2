...
type
  ...
  public
    class function Execute(const ACaption: String; AWave: TWave;
...
begin
  ...
  RegisterPropertyEditor(TypeInfo(TWave), nil, '', TWavePropertyEditor);
...
class function TWavePropertyDialog.Execute(const ACaption: String;
  AWave: TWave; var AFileName: String): Boolean;
begin
  Result := False;
  with Create(Application) do
    try
      ...
      begin
        if Assigned(AWave) then
          AWave.LoadFromStream(Player.Wave);
        AFileName := FileName;
        Result := True;
      end;
    finally
...
begin
  ...
  ContentSize.Caption := FormatFloat('#,##0 bytes', Player.Wave.Size);
  btnClear.Enabled := Player.Wave.Size > 0;
  btnSave.Enabled := Player.Wave.Size > 0;
